...
.h2o.transformParam <- function(paramDef, paramValue) {
  # Fetch mapping for given Java to R types
  mapping <- .type.map[paramDef$type,]
  type    <- mapping[1L, 1L]
  scalar  <- mapping[1L, 2L]
...
.h2o.checkHyperParameters <- function(algo, allParams, hyper_params) {

  error <- lapply(allParams, function(paramDef) {
      e <- ""
      name <- paramDef$name
      ...
      if (!paramDef$gridable && (name %in% hyper_names)) {
        ...
        e <- .h2o.checkParam(paramDef, params)
      }
      e
  })
